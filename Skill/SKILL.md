...
|------|----|----|-------------------|-----------------|------------|
| (1) **Observational** | yes | never | IG+SP+PHY+LL+CR (offline) | no | not required |
| (2) **Active** | yes | yes with per-command re-check | full up to AT (AP if controlled) | no | recommended if TX present |
| (3) **Lab** | yes | yes with per-command re-check | full, incl. AT/AP | no | **mandatory** (cage/conducted) |
| (4) **Defensive** | yes | never | no | yes detect->correlate->alert | not required |
...
### RFSAM modes x layers matrix (what you do per layer depending on mode)

| Layer | Observational | Active | Lab | Defensive |
|------|---------------|--------|-----|-----------|
| IG | yes CVE/chipset/FCC ID | yes | yes | yes (asset to defend) |
| SP | yes RX survey | yes | yes | yes threat survey |
| PHY | yes offline demod | yes | yes | yes decode attacker emission |
| LL | yes captured frames | yes | yes | yes detect anomalous frames |
| CR | yes key from captured data | yes | yes | [!] only if attacker breaks the defended link's crypto |
| AT | no | yes TX re-check | yes TX re-check + containment | no |
| AP | no (only BTC has control) | yes if controlled | yes | no |

### TX re-check - before ANY command that transmits (not only at AT)

Read `loot/scope.txt`, confirm `mode in {active, lab}` and that the command is within the authorized scope. If not,
...
> `protocol=SDR-general` and pin the canonical one upon confirming it at SP - see `02-kit-sdr.md Sec Subflow`.
...
- **Action:**
  ...
  - Verify the environment (5 checks - see `references/25-troubleshooting.md` Sec setup). Result -> `loot/notes/hardware.txt`.
- **Exit criterion:**
  -  yes Protocol confirmed and wayfinder loaded.
  -  yes Applicable controls listed; `loot/scope.txt` created with mode persisted.
  -  yes Required hardware identified (present or gap declared).
...
- **Exit criterion:**
  -  yes Encryption status evaluated (algorithm + strength).
  -  yes If encrypted: recovery feasibility documented (successful or not, with evidence).
  -  yes Keys (if any) in `loot/keys/`, not in chat.

### Phase 5 - AT (Attack) - TX re-check mandatory

- **Precondition:** Phase 4 complete. **TX re-check** (see SCOPE AND LIMITS): before ANY TX command, read `loot/scope.txt`, confirm `mode in {active, lab}` and that the command is in scope; apply the legal tier (T1/T2 = stop unless Lab+containment; T3 = authorized active; T4 = gap). If not met -> stop and ask for confirmation.
- **Action:** Observational -> document vectors as **hypotheses**, do not execute TX. Active/lab -> execute injection/replay/hijack/rogue infrastructure per the protocol and AT controls. **Critical infrastructure** (GNSS/ADS-B/rogue cell): Lab with containment only - "in the field" = rejection (RA4/RA5/RA8). Verbatim commands and dual-use warnings: `references/NN-proto.md Sec AT`. Register each attack with evidence.
- **Exit criterion:**
  -  yes TX re-check completed for each TX command executed.
  -  yes Vectors documented (executed or as hypotheses depending on mode).
  -  yes AT findings registered with reproducible evidence; AT controls covered or gap declared.

### Phase 6 - AP (Application)

- **Precondition:** Phase 5 complete.
- **Action:** Only if the protocol has an AP control (mainly BTC; most do not have an AP layer - "not applicable" is a valid closure). Evaluate what the device trusts over the link: profiles, services, application data. Steps: `references/NN-proto.md Sec AP` if it exists.
- **Exit criterion:**
  -  yes AP evaluated or "not applicable for this protocol" justified.
  -  yes AP findings registered (if any).
...
- **Exit criterion:**
  -  yes Closure checklist complete (all items  yes or justified).
  -  yes Technical report and executive summary generated.
  -  yes `loot/scope.txt` finalized (closure date, retention confirmed).
...
**Decision by 4-axis model** (Impact x Exploitability x Exposure x Scope A/B/C/D), complete decision table
and 13 worked examples: `references/03-finding-registration.md Sec rf-severity`. **Golden rules:** without PoC (Scope C) the
maximum is `medium`; cage (B) lowers `critical`->`high` (label `contained`); Defensive (D) never reports `critical`
(type `detection`). The model produces the severity; Sec EVIDENCE verifies that the evidence supports it, or degrades it.
...
`references/03-finding-registration.md Sec 5`. **Exposure and Scope live in the JSONL**, not in the CVSS vector (CVSS does not
...
with Operator only (see `references/03-finding-registration.md Sec 7`).

**Before registering**, run the Q1-Q8 checklist (`references/26-quality.md Sec pre-registration`); if any item is
...
**Closure checklist - per finding:** Q1-Q8 rubric passed (`references/26-quality.md Sec pre-registration`); `repro.txt` + verbatim
command in `loot/poc/RF-NNN/` (Sec EVIDENCE); 4-axis model + CVSS 4.0 (Sec SEVERITY); `RFSAM-<PROTO>-<LAYER>-NN` control mapped;
...
retention confirmed); visibility gaps declared; PII sanitized in evidence and report (PII policy in Sec SCOPE).

**Deliverables:**
1. `python3 scripts/coverage_check.py` -> lists covered vs. pending controls per protocol (dump to report Sec 5).
