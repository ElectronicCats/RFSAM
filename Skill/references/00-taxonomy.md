...
> (protocol x layer), the IDs, the criticality rubric, the full coverage-map, and the
...
`confidence` in `low medium high` - honest self-assessment of the draft.

## 6. Full coverage-map

Map of all controls that RFSAM defines (one per relevant protocolxlayer cell).
