...
> crypto strength. Source: Sec 1 (setup), Sec 2 (diagnosis), Sec 3 (order of diagnosis), Sec 4 (false positives).

## Index
1. Sec setup - 5 environment checks (Phase 0, cache result in `loot/notes/hardware.txt`)
2. Sec diagnosis - symptom -> probable cause -> action table
3. Sec order - diagnosis rule (antenna before driver before binary)
4. Sec false-positives - finding that looks confirmed but is not

---

## 1. Sec setup - 5 environment checks (Phase 0)
...
| SDR in `lsusb` but binary returns `Permission denied` | Missing `dialout`/`plugdev` group or udev rule | Sec 1.3; `usermod -aG dialout,plugdev $USER` + relogin; reinstall package to copy udev rule |
...
> **Diagnosis rule (Sec 3)**: on "I can't see the signal", check in THIS order - antenna (Sec 1.4) -> gain/overflow (here) -> driver (Sec 1.3) -> binary (Sec 1.1) -> band gap. Antenna and gain explain 80% of cases.

| Symptom | Probable cause | Action |
|---------|----------------|--------|
| `gqrx` shows flat noise with hardware OK | Antenna absent/incorrect, bias-T off (GNSS), mistuned dipole | Sec 1.4 first; `rtl_biast -b 1` for GNSS; ask operator about connected antenna |
...
| `dump1090` reports 0 messages/min | 1090 antenna without LNA/filter or bad orientation | Sec 1.4 ADS-B; vertical quarter-wave antenna + LNA + 1090 filter |
...
| Analysis on PCAP with overflows != 0 | Base capture silently incomplete | Re-capture (safe-capture Sec 4); overflows cause garbage to be decoded and presented as a finding |
...
## 3. Sec order - diagnosis rule

On "I can't see / it doesn't work", do NOT declare a gap immediately. Follow this order:

1. **Antenna** (Sec 1.4) - is it connected and correct for the band? `gqrx` with flat noise + hardware OK = antenna first.
2. **Gain/overflow** (Sec 2.2) - is it saturating or buried? Adjust before declaring "no signal".
3. **Driver/permissions** (Sec 1.3) - does the device open? `Permission denied` != broken hardware.
4. **Binary/decoder** (Sec 1.1, Sec 2.3) - are the right tool and decoder present? `which`, Wireshark version.
...
Before registering, discard the typical false positive of the pattern (see Sec 4 below, "Typical false positive"
...
- **SKILL.md Phase 0** cites Sec setup as the body of the environment check.
- **SKILL.md Route B** cites Sec diagnosis as the step before escalating.
- **Wayfinders** (`references/NN-proto.md`) may cite "see troubleshooting Sec 2.2" for the specific family.
- **Phase 7.1 (validation):** every "it doesn't work" documented in `loot/notes/` must reference Sec order - without
